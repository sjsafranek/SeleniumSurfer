...
<seelog minlevel="` + LogLevel + `">
    ...
    <formats>
        <format id="common"   format="[SeleniumSurfer] %UTCDate %UTCTime [%LEVEL] %File %FuncShort:%Line %Msg %n" />
        <format id="stdout"   format="[SeleniumSurfer] %UTCDate %UTCTime [%LEVEL] %File %FuncShort:%Line %Msg %n" />
    </formats>
</seelog>
`
